...
class Ringer:
    def __init__(self):
        self.input_data = mod.LoadData(publication='The Ringer')

    def get_posts(self, soup: HTMLParser):
        articles = soup.css('div[class="c-compact-river__entry "]')
        return articles
    ...
    def get_data_from_post(self, post: HTMLParser, author_name: str):
        ...
        if mod.less_than_3_days_old(date):
            ...
            if post.css_first('p[class="p-dek c-entry-box--compact__dek"]'):
                paragraph = post.css_first('p[class="p-dek c-entry-box--compact__dek"]').text()
            if post.css_first('h2 a'):
                link = post.css_first('h2 a').attributes['href']
                response = mod.send_requests(link)
                if response:
                    soup = HTMLParser(response.text)
                    if soup.css_first('picture img'):
                        img = soup.css_first('picture img').attributes['src']
                authors = post.css('span[class="c-byline__item"] a')
