...
class NyPost:
    def __init__(self):
        self.input_data = mod.LoadData(publication='New York Post')

    def get_posts(self, soup: HTMLParser):
        articles = soup.css('div[class="layout__item"]')
        return articles
    ...
    def get_data_from_post(self, post: HTMLParser, author_name: str):
        ...
        if mod.less_than_3_days_old(date):
            ...
            if post.css_first('p[class="story__excerpt body"]'):
                paragraph = post.css_first('p[class="story__excerpt body"]').text(strip=True)
