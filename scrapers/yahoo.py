...
class Yahoo:
    def __init__(self):
        self.input_data = mod.LoadData(publication='Yahoo')

    def get_posts(self, soup: HTMLParser):
        articles = soup.css('div[data-test-locator="mega"]')
        return articles
    ...
    def get_data_from_post(self, post: HTMLParser, author_name: str):
        ...
        if post.css_first('h3 a'):
            link = post.css_first('h3 a').attributes['href']
            response = mod.send_requests(link)
            if response:
                soup = HTMLParser(response.text)
                if soup.css_first('div[class="caas-attr author"] time'):
                    date = soup.css_first('div[class="caas-attr author"] time').text(strip=True)
                if mod.less_than_3_days_old(date):
                    if soup.css_first('h1[data-test-locator="headline"]'):
                        header = soup.css_first('h1[data-test-locator="headline"]').text(strip=True)
                    if soup.css_first('div[class="caas-body"] p'):
                        paragraph = soup.css_first('div[class="caas-body"] p').text(strip=True)
                    authors = soup.css('span[class="caas-author-byline-collapse"] a')
